...
check(
  'source challenge drives player step',
  new Source().nextStep(srv('source', 'x:1'), hist('info', info)) !== null,
);
...
check(
  'info_retry appends the 4-byte challenge',
  retryStep !== null && retryStep.packet.subarray(-4).equals(Buffer.from([0x44, 0x33, 0x22, 0x11])),
);
...
check(
  'info parsed from the challenge-completed reply',
...
check(
  'the ship: extra 3 bytes consumed',
  ship.ship_mode === 2 && ship.ship_witnesses === 3 && ship.ship_duration === 4,
);
check('the ship: version still aligned after extra bytes', ship.version === '1.0.0.4');

console.log('\nMinecraft framing + JSON');
const mcJson = JSON.stringify({
...
len.writeUInt16BE(motd.length, 0);
...
console.log('\nPalworld / FiveM HTTP');
const palInfo = Buffer.from(
  'HTTP/1.1 200 OK\r\nContent-Length: 46\r\n\r\n{"servername":"Test Pal","version":"v0.4.1"}',
  'utf8',
);
...
check(
  'palworld 401 -> auth_error',
  new Palworld().parse(srv('palworld', 'x:8212'), hist('info', pal401)).auth_error === true,
);
const fmInfoBody = JSON.stringify({
  server: 'FX',
  vars: { sv_projectName: 'Test RP', sv_maxClients: '48', mapname: 'SA', gametype: 'RP' },
});
const fmPlayersBody = JSON.stringify([{ name: 'Alice' }, { name: 'Bob' }]);
const fmInfo = Buffer.from(
  `HTTP/1.1 200 OK\r\nContent-Length: ${Buffer.byteLength(fmInfoBody)}\r\n\r\n${fmInfoBody}`,
  'utf8',
);
const fmPlayers = Buffer.from(
  `HTTP/1.1 200 OK\r\nContent-Length: ${Buffer.byteLength(fmPlayersBody)}\r\n\r\n${fmPlayersBody}`,
  'utf8',
);
const fp = new FiveM().parse(srv('fivem', 'x:30120'), [
  ...hist('info', fmInfo),
  ...hist('players', fmPlayers),
]);
...
check(
  'quake3 players parsed',
  q3p.players === 2 && JSON.stringify(q3p.players_list) === JSON.stringify(['P1', 'P2']),
);
...
console.log('\nGameSpy 1 / 2 / 3');
const gs1 = Buffer.from(
  '\\hostname\\My UT\\mapname\\CTF-Face\\maxplayers\\16\\player_0\\Loque\\player_1\\Xan\\final\\',
  'latin1',
);
const gs1p = new GameSpy1().parse(srv('gamespy1', 'x:7778'), hist('status', gs1));
check(
  'gamespy1 name/players parsed',
  gs1p.name === 'My UT' && JSON.stringify(gs1p.players_list) === JSON.stringify(['Loque', 'Xan']),
);
const gs2 = Buffer.from(
  '\x00\x04\x05\x06\x07hostname\x00My Halo\x00mapname\x00Blood Gulch\x00maxplayers\x0016\x00numplayers\x002\x00\x00\x02player_\x00score_\x00Chief\x00100\x00Cortana\x0080\x00\x00',
  'latin1',
);
...
const beChal = Buffer.alloc(4);
beChal.writeInt32BE(1234567, 0);
check('gamespy3 challenge is big-endian', gs3Next !== null && gs3Next.packet.includes(beChal));
const gs3Info = Buffer.from(
  '\x00\x04\x05\x06\x07splitnum\x00\x00hostname\x00My BF2\x00mapname\x00Strike\x00maxplayers\x0064\x00numplayers\x002\x00\x00\x01player_\x00\x00Alice\x00Bob\x00\x00',
  'latin1',
);
...
console.log('\nUnreal 2');
const ustr = (s: string): Buffer =>
  Buffer.concat([Buffer.from([s.length + 1]), Buffer.from(s + '\x00', 'utf8')]);
const i32 = (n: number): Buffer => {
  const b = Buffer.alloc(4);
  b.writeInt32LE(n, 0);
  return b;
};
const u2 = Buffer.concat([
  Buffer.from([0x80, 0, 0, 0, 0]),
  i32(1),
  ustr('1.2.3.4'),
  i32(7777),
  i32(7787),
...
const d3 = Buffer.concat([
  ...
  Buffer.from(
    'si_name\x00My Doom3\x00si_map\x00d3dm1\x00si_maxPlayers\x008\x00si_version\x00DOOM 1.3.1\x00\x00',
    'latin1',
  ),
  Buffer.from([0x00, 0x0f, 0x00, 0, 0, 0, 0]),
  Buffer.from('Marine\x00', 'latin1'),
  Buffer.from([0x20]),
]);
...
check(
  'ase name/map/max parsed',
  asep.name === 'My MTA' && asep.map === 'San Andreas' && asep.max_players === 50,
);
check(
  'ase players parsed',
  asep.players === 2 && JSON.stringify(asep.players_list) === JSON.stringify(['Alice', 'Bob']),
);

console.log('\nMumble');
const mumble = Buffer.alloc(24);
mumble.writeUInt8(0, 0);
mumble.writeUInt8(1, 1);
mumble.writeUInt8(4, 2);
mumble.writeUInt8(31, 3); // version 1.4.31
...
check(
  'mumble version/users/max parsed',
  mp.version === '1.4.31' && mp.players === 42 && mp.max_players === 100,
);
check('mumble bandwidth parsed', mp.bandwidth === 72000);
check(
  'mumble short packet rejected',
  Object.keys(new Mumble().parse(srv('mumble', 'x:64738'), hist('ping', Buffer.from([0, 1])))).length === 0,
);
...
check(
  'frostbite name/players/max parsed',
  fbp.name === 'My BF4 Server' && fbp.players === 32 && fbp.max_players === 64,
);
...
check(
  'frostbite request framing well-formed',
  fb.isResponseComplete(fb.initialStep(srv('frostbite', 'x:47200')).packet) === true,
);
...
check(
  'assettocorsa players/max/password parsed',
  acp.players === 12 && acp.max_players === 24 && acp.password === true,
);
check(
  'assettocorsa framing gated by content-length',
  ac.isResponseComplete(acResponse.subarray(0, acResponse.length - 5)) === false,
);
check('assettocorsa framing complete', ac.isResponseComplete(acResponse) === true);
check(
  'assettocorsa non-200 rejected',
  Object.keys(
    ac.parse(
      srv('assettocorsa', 'x:8081'),
      hist('info', Buffer.from('HTTP/1.1 404 Not Found\r\nContent-Length: 0\r\n\r\n', 'latin1')),
    ),
  ).length === 0,
);
...
check(
  'teamspeak3 name/max/version parsed',
  tsp.name === 'My Cool TS3 Server' && tsp.max_players === 32 && tsp.version === '3.13.7',
);
check('teamspeak3 players exclude query clients', tsp.players === 5);
check(
  'teamspeak3 completion needs two error lines',
  ts.isResponseComplete(Buffer.from('TS3\r\nerror id=0 msg=ok\r\n', 'latin1')) === false,
);
check('teamspeak3 completion after serverinfo', ts.isResponseComplete(tsResponse) === true);
check(
  'teamspeak3 request selects voice port',
  ts
    .initialStep(Server.fromAddress('teamspeak3', 'x:10011', null, { voicePort: 9988 }))
    .packet.toString('latin1')
    .includes('use port=9988'),
);
...
check(
  'terraria name/world/version parsed',
  trp.name === 'My Terraria World' && trp.map === 'Hallow' && trp.version === '1.4.4.9',
);
...
const sampHeader = (op: string): Buffer =>
  Buffer.concat([
    Buffer.from('SAMP', 'latin1'),
    Buffer.from([127, 0, 0, 1]),
    leU16(7777),
    Buffer.from(op, 'latin1'),
  ]);
...
check(
  'samp name/gametype/language parsed',
  sampp.name === 'Los Santos Roleplay' && sampp.gametype === 'Freeroam' && sampp.language === 'English',
);
check(
  'samp players/max/password parsed',
  sampp.players === 50 && sampp.max_players === 100 && sampp.password === false,
);
...
check(
  'samp request has SAMP header + opcode',
  sampReq.subarray(0, 4).toString('latin1') === 'SAMP' && sampReq[10] === 0x69,
);
check(
  'samp request embeds ip octets',
  JSON.stringify([...sampReq.subarray(4, 8)]) === JSON.stringify([127, 0, 0, 1]),
);
...
check(
  'samp uses resolved ip in packet',
  JSON.stringify([...samp.initialStep(sampResolved).packet.subarray(4, 8)]) ===
    JSON.stringify([203, 0, 113, 5]),
);
...
check(
  'quakeworld hostname/map/max parsed',
  qwp.name === 'Frag Palace' && qwp.map === 'dm6' && qwp.max_players === 16,
);
check(
  'quakeworld players parsed',
  qwp.players === 2 && JSON.stringify(qwp.players_list) === JSON.stringify(['Ranger', 'Visor']),
);
check('quakeworld gamedir/version exposed', qwp.game === 'qw' && qwp.version === 'ezQuake');
check(
  'quakeworld request is OOB status',
  qw
    .initialStep(srv('quakeworld', 'x:27500'))
    .packet.equals(Buffer.from('\xff\xff\xff\xffstatus\n', 'latin1')),
);
...
check(
  'minecraft-legacy motd/version/protocol parsed',
  mclp.name === 'My Legacy Server' && mclp.version === '1.6.4' && mclp.protocol_version === 127,
);
check('minecraft-legacy players/max parsed', mclp.players === 7 && mclp.max_players === 20);
check(
  'minecraft-legacy framing gated by length',
  mcl.isResponseComplete(mclResponse.subarray(0, 5)) === false,
);
check('minecraft-legacy framing complete', mcl.isResponseComplete(mclResponse) === true);
check(
  'minecraft-legacy request is FE01',
  mcl.initialStep(mclServer).packet.equals(Buffer.from([0xfe, 0x01])),
);
...
check(
  'server label falls back to host:port',
  new Server('source', '1.2.3.4', 27015).label() === '1.2.3.4:27015',
);

const v6 = Server.fromAddress('source', '[::1]:27015');
check('fromAddress parses bracketed IPv6 (brackets stripped)', v6.host === '::1' && v6.port === 27015);
const v6b = Server.fromAddress('bedrock', '[2001:db8::1]:19132');
...
check(
  'fromAddress still parses a hostname',
  Server.fromAddress('minecraft', 'mc.example.com:25565').port === 25565,
);
...
check(
  'offline accessors are null-safe',
  offlineResult.name() === null &&
    offlineResult.players() === null &&
    offlineResult.playerNames().length === 0,
);
