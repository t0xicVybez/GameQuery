 ...
 *
 * 'gamePort' is present only for games whose join port differs from the query
 * port (e.g. Killing Floor 2 is queried on 27015 but joined on 7777), so a
 * caller can show "connect on X" without querying the wrong port. It is
 * advisory only and never queried.
 *
 * Kept 1:1 with the Node port's Games map (enforced by tests/parity-check.sh).
 */
final class Games
{
    /** @var array<string, array{protocol: string, port: int, gamePort?: int, name: string}> */
    public const GAMES = [
        // Source / A2S
     ...
     *
     * @return array{protocol: string, port: int, gamePort?: int, name: string}|null
     */
